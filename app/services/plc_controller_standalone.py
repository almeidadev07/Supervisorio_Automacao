...
class StandalonePLCController:
        ...
        for machine_config in self.machines_config:
            ...
            if os.path.exists(comm_map_file):
                try:
                    ...
                    print(f"[STANDALONE_PLC] 📋 Comm map carregado para {machine_name}: {len(comm_map)} tags")
                except Exception as e:
                    print(f"[STANDALONE_PLC] ❌ Erro ao carregar comm_map para {machine_name}: {e}")
                    self._comm_map_by_machine[machine_name] = []
            else:
                print(f"[STANDALONE_PLC] ⚠️ Comm map não encontrado para {machine_name}")
                self._comm_map_by_machine[machine_name] = []
    ...
    def _create_driver(self):
        ...
        try:
            ...
            if self._driver.connect():
                ...
                print(f"[STANDALONE_PLC] ❌ Falha na conexão do driver real")
                self._driver = None
                return False
    ...
    def _attempt_reconnection(self):
        ...
        if self._reconnect_attempts >= self._max_reconnect_attempts:
            print(f"[STANDALONE_PLC] ❌ Máximo de tentativas de reconexão atingido")
            return False
        ...
        try:
            ...
            if self._create_driver():
                ...
                print(f"[STANDALONE_PLC] ❌ Falha na reconexão")
                return False
    ...
    def _read_from_plc(self, tags: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = str(e)
            print(f"[STANDALONE_PLC] ❌ Erro ao ler do PLC: {error_msg}")
    ...
    def set_active_machine(self, cfg):
        ...
        with self._lock:
            ...
            print(f"[STANDALONE_PLC] 🏭 Máquina ativa: {cfg.get('name')}")
    ...
    def unsubscribe_client(self, client_id: str) -> bool:
        ...
        with self._subscription_lock:
            if client_id in self._subscriptions:
                ...
                print(f"[STANDALONE_PLC] 🗑️ Cliente {client_id} removido das subscrições")
                return True
            return False
